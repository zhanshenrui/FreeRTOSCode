...
#include "stdlib.h"
//LCD ID:0x9325
//LCD宽为240，高为303，即x最大为232(起始为232+8(字宽)=240)，y最大为303(303+16=320)
//当用16的size时(宽为8，高为16)，一行可以写240/8=30个字母，可以写320/16=20行
//////////////////////////////////////////////////////////////////////////////////	 
...
//注意设置时STM32内部会右移一位对其! 			    
#define LCD_BASE      	((u32)(0x60000000 | 0x0C000000))//((u32)(0x6C000000 | 0x000007FE))
...
/*
3.2寸屏:240*320像素，每个像素占18位，但我们一般用16位模式，即RGB=565
叠加方式:在显示的图片上再显示字符，一般用非叠加方式
字体大小:1206,1608,2412三种大小,1608即高为16，宽为8
//用1608大小的话，一行宽240，每个字符占8，总共30个字符
//高为16，高度为320，即可以有20行，但最后一行显示不正常，所以只有
//19行，从0开始计数，只能到18行

LCD_ShowString(0,16*5,8*strlen((char *)USART_RX_BUF),16,16,USART_RX_BUF);
strlen((char *)USART_RX_BUF)这里用了强制转换是因为USART_RX_BUF是u8类型，而strlen()要求
的参数是字符串,u8*类型，所以将其强制转为指针
LCD_ShowString(0,16*5,8*strlen((char *)USART_RX_BUF),16,16,USART_RX_BUF);
用的尺寸大小是1608，即高16，宽8
每行高16，所以在第5行用16*5，x坐标从0开始，每个字母宽是8，所以字符串
宽度是8*strlen((char *)USART_RX_BUF),高为16，大小为16
